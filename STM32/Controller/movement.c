...
int standardize_y(float y_value) {
    ...
    {
        if (y_value >= 0) return (int)(100 * y_value / (4095 - Y_MIDPOINT));
        else if (y_value < 0) return (int)(100 * y_value / (Y_MIDPOINT));
    }

    return 0;
}

int standardize_x(float x_value) {
    // checks if ADC is more than 5% higher than the initial value before starting to move
    if (x_value > MINIMUM_PERCENT_ACTIVE * (4095 - X_MIDPOINT) || x_value < - MINIMUM_PERCENT_ACTIVE * X_MIDPOINT) {
        if (x_value >= 0) return (int)(100 * x_value / (X_MIDPOINT));
        else if (x_value < 0) return (int)(100 * x_value / (4095 - X_MIDPOINT));
    }
