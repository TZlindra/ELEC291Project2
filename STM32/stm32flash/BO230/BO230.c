...
// CBUs_3 and CBUs0 are used as RESET and BOOT respectevely, to activate bootload mode.
...
void print_help (char * prn)
{
	printf("Some examples:\n"
	       "%s -b (activate bootloader using CBUs_3 as RESET and CBUS0 as BOOT)\n"
...
int main(int argc, char ** argv)
{
	...
	FT_SetBitMode(handle, 0x00, FT_BITMODE_CBUS_BITBANG); // Set CBUs_3 as input
	FT_SetBitMode(handle, 0x0, FT_BITMODE_RESET); // Back to serial port mode
	FT_Close(handle);
