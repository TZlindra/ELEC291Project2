...
ComBuffer ComRX_BUFFer, ComTXBuffer;
...
int ReadCom(int Max,unsigned char *Buffer)
{
	...
	unsigned i;
  	if (!ComOpen)
    	return (-1);
	i=0;
	while ((i < Max-1) && (GetBufCount(&ComRX_BUFFer)))
		Buffer[i++] = GetBuf(&ComRX_BUFFer);
	if (i>0)
	{
		Buffer[i]=0;
		return(i);
...
void initUART(int BaudRate)
{
	...
	//int j;
	 ...
	 __disable_irq();
	ComRX_BUFFer.Head = ComRX_BUFFer.Tail = ComRX_BUFFer.Count = 0;
...
void usart_rx (void)
{
	...
	if ( PutBuf(&ComRX_BUFFer, USART1->RDR) )
	{
...
int egets(char *s, int Max)
{
	...
	{
		while (!GetBufCount(&ComRX_BUFFer)); // wait for a character
		c = GetBuf(&ComRX_BUFFer);
		s[Len++] = c;
	}
	if (Len>0)
	{
